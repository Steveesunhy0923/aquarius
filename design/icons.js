const I={
 ...
 // Text formatting — drawn letterforms (format-icons.html); bold gets a heavier stroke via THICK.
 ...
 live:'<circle cx="12" cy="12" r="3.2" style="fill:currentColor"/>',
 ...
 // Graph editor tools, visibility toggles & list markers (graph-tool-icons.html).
...
const THICK={bold:2.3};
function svg(i){return '<svg viewBox="0 0 24 24">'+(i||'')+'</svg>';}
function hydrate(root){
  (root||document).querySelectorAll('[data-ic]').forEach(el=>{el.classList.add('ic');el.innerHTML=svg(I[el.dataset.ic]);const w=THICK[el.dataset.ic];if(w)el.firstElementChild.style.strokeWidth=w;});
}
